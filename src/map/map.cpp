...
int map_search_freecell(struct block_list *src, int16 m, int16 *x,int16 *y, int16 rx, int16 ry, int flag)
{
	...
	while(tries--) {
		*x = (rx >= 0) ? rnd_value(bx - rx, bx + rx) : rnd_value<int16>(5, mapdata->xs - 5);
		*y = (ry >= 0) ? rnd_value(by - ry, by + ry) : rnd_value<int16>(5, mapdata->ys - 5);
...
void map_flags_init(void){
	...
	mapflag_config.insert(std::make_pair(MF_NOMOBLEVELUP, s_mapflag_item{
		/* 地图标记名称 (主要用在 @mapinfo 指令中显示) */ "NoMobLevelUp",
		/* 当有参数值的时候, 若全部参数的值等于默认值时, 是否自动关闭此地图标记 */ false,
		/* 禁止在 @mapflag 指令中开启此地图标记 */ false,
		/* 参数列表定义(支持多参数), 格式: {默认值, 最小值, 最大值, <"可选: 参数单位">} */ {
			{0, 0, 1}
	...
	mapflag_config.insert(std::make_pair(MF_NOMAGICIMMUNITY, s_mapflag_item{
		/* 地图标记名称 (主要用在 @mapinfo 指令中显示) */ "NoMagicImmunity",
		/* 当有参数值的时候, 若全部参数的值等于默认值时, 是否自动关闭此地图标记 */ false,
		/* 禁止在 @mapflag 指令中开启此地图标记 */ false,
		/* 参数列表定义(支持多参数), 格式: {默认值, 最小值, 最大值, <"可选: 参数单位">} */ {
			{0, 0, 1}
	...
	mapflag_config.insert(std::make_pair(MF_NOGSTACK, s_mapflag_item{
		/* 地图标记名称 (主要用在 @mapinfo 指令中显示) */ "NoGStack",
		/* 当有参数值的时候, 若全部参数的值等于默认值时, 是否自动关闭此地图标记 */ false,
		/* 禁止在 @mapflag 指令中开启此地图标记 */ false,
		/* 参数列表定义(支持多参数), 格式: {默认值, 最小值, 最大值, <"可选: 参数单位">} */ {
			{0, 0, 1}
