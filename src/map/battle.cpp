	...
	if (bl->type == BL_PC) {
		...
		if(flag&BF_MAGIC && sd->special_state.no_magic_damage) {
			if (tsc && !tsc->getSCE(SC_DEADLY_DEFEASANCE)) //put it here because in in pc_calc_sub with CalcFlag All it'll break setunitdata of monsters, with CalcFlag Base it'll break other sc's bonuses [datawulf]
				damage -= damage * min(map_getmapflag(bl->m, MF_NOMAGICIMMUNITY) ? 95 : 100, sd->special_state.no_magic_damage) / 100;
		}
...
static struct Damage battle_calc_weapon_attack(struct block_list *src, struct block_list *target, uint16 skill_id, uint16 skill_lv, int wflag)
{
	...
	if(!is_attack_hitting(&wd, src, target, skill_id, skill_lv, true))
		...
		if (sd) { //monsters, homuns and pets have their damage computed directly
			...
			if (skill_id == 0)
				ATK_ADDRATE(wd.damage, wd.damage2, sd->bonus.melee_rate);
		}
	...
	if (skill_id && bl && map_getmapflag(bl->m, MF_MAXDMG_SKILL)) {
		...
		if (val > 0 && d.damage + d.damage2 > val) {
			...
			if (d.damage2 >= overval) {
				...
				overval -= d.damage2;	// ¸üÐÂ³¬³öµÄÉËº¦Êý
				d.damage2 = 0;
				d.damage = cap_value(d.damage - overval, 0, val);	// ×îºóÔÙ¿Û¼õ damage ÖÐµÄÉËº¦
			}
		}
	}
...
enum damage_lv battle_weapon_attack(struct block_list* src, struct block_list* target, t_tick tick, int flag) {
	...
	if (sc && sc->getSCE(SC_AUTOSPELL) && rnd()%100 < (sc->getSCE(SC_AUTOSPELL)->val4 * (sstatus->luk + 100) / 100 * (100 + sd->bonus.autospell_rate) / 100)) {
		int sp = 0;
	...
	if (sd) {
		...
		if( wd.flag&BF_WEAPON && sc && sc->getSCE(SC__AUTOSHADOWSPELL) && rnd()%100 < (sc->getSCE(SC__AUTOSHADOWSPELL)->val3 * (100 + sstatus->luk * 3 / 2) / 100 * (100 + sd->bonus.autospell_rate) / 100) &&
		...
		if( sc ){
			...
			if( sc->getSCE(SC_ABYSSFORCEWEAPON) && sd->abyssball > 0 && rnd() % 100 < (15 * (sstatus->luk + 100) / 100) ){
			...
			if( sc->getSCE(SC_ABYSSFORCEWEAPON) && rnd() % 100 < (20 * (sstatus->luk + 100) / 100)){
...
// rAthena
	...
	// ³öÏÖÁËÖØ¸´¶¨Òå, ÕâÀïÏÈ¼òµ¥×¢ÊÍµô, Èç¹û rAthena ¹Ù·½ºÜ¾Ã²»¸ÄµÄ»°, ÕÒ¸öÊ±¼äÔÙÌá½»¸ö PullRequest ÐÞÕýÏÂ [SolaØ¼Ð¡¿Ë]
	{ "fame_refine_lv1",                    &battle_config.fame_refine_lv1,                 1,      0,      INT_MAX,        },
...
#ifdef Pandas_BattleConfig_MaxAspdForPVP
	// ¸ù¾Ý max_aspd_for_pvp Ô¼ÊøÍæ¼ÒµÄ×î´ó¹¥ËÙ [SolaØ¼Ð¡¿Ë]
	// ÕâÀï¶ÔÅäÖÃµÄ ASPD ÊýÖµ (ÀýÈç: 193, 197) ×ª»»³É³ÌÐòÅÐ¶ÏÓÃµÄ¹¥»÷¼ä¸ôÊ±¼ä
	// 
	// ¹¥»÷¼ä¸ô = 2000 - ¹¥ËÙµÄ Aspd ÊýÖµ * 10
	// ¹¥»÷¼ä¸ô = 2000 - 193 * 10
	// ¹¥»÷¼ä¸ô = 2000 - 1930
	// ¹¥»÷¼ä¸ô = 70 ºÁÃë
...
#ifdef Pandas_BattleConfig_MaxAspdForGVG
	// ¸ù¾Ý max_aspd_for_gvg Ô¼ÊøÍæ¼ÒµÄ×î´ó¹¥ËÙ [SolaØ¼Ð¡¿Ë]
	// ÕâÀï¶ÔÅäÖÃµÄ ASPD ÊýÖµ (ÀýÈç: 193, 197) ×ª»»³É³ÌÐòÅÐ¶ÏÓÃµÄ¹¥»÷¼ä¸ôÊ±¼ä
	// 
	// ¹¥»÷¼ä¸ô = 2000 - ¹¥ËÙµÄ Aspd ÊýÖµ * 10
	// ¹¥»÷¼ä¸ô = 2000 - 193 * 10
...
int battle_config_read(const char* cfgName)
{
	...
	if (count == 0) {
		...
		static const struct _battle_config_check_whitelist {
			...
			{ "traps_setting" },
			{ "item_enabled_npc" },
			{ "guild_skill_relog_type" },				// ²»Í¬¹¤×÷Ä£Ê½ÏÂÓµÓÐ²»Í¬µÄÄ¬ÈÏÖµ, Ñ¡ÏîÄ¬ÈÏ´¦ÓÚ×¢ÊÍ×´Ì¬
			{ "feature.instance_allow_reconnect" },		// ²»Í¬¹¤×÷Ä£Ê½ÏÂÓµÓÐ²»Í¬µÄÄ¬ÈÏÖµ, Ñ¡ÏîÄ¬ÈÏ´¦ÓÚ×¢ÊÍ×´Ì¬
